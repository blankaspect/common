...
////////////////////////////////////////////////////////////////////////
	...
	private static final	int	MIN_HEADER_WIDTH	= SCROLL_BUTTON_LEADING_MARGIN +
														...
														SCROLL_BUTTON_TRAILING_MARGIN + LIST_BUTTON_WIDTH +
	...
	private static final	ImageIcon	ACTIVE_CROSS_ICON	= new ImageIcon(ImageData.ACTIVE_CROSS);
	private static final	ImageIcon	CORNER_L_ICON		= new ImageIcon(ImageData.CORNER_L);
	private static final	ImageIcon	CORNER_R_ICON		= new ImageIcon(ImageData.CORNER_R);
	private static final	ImageIcon	CORNER_LS_ICON		= new ImageIcon(ImageData.CORNER_LS);
	private static final	ImageIcon	CORNER_RS_ICON		= new ImageIcon(ImageData.CORNER_RS);
	...
	private interface ImageData
	{
		...
		// Image: tabbedPanel-corner-l.png
		byte[]	CORNER_L	=
		{
			(byte)0x89, (byte)0x50, (byte)0x4E, (byte)0x47, (byte)0x0D, (byte)0x0A, (byte)0x1A, (byte)0x0A,
			(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x0D, (byte)0x49, (byte)0x48, (byte)0x44, (byte)0x52,
		...
		// Image: tabbedPanel-corner-r.png
		byte[]	CORNER_R	=
		{
			(byte)0x89, (byte)0x50, (byte)0x4E, (byte)0x47, (byte)0x0D, (byte)0x0A, (byte)0x1A, (byte)0x0A,
			(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x0D, (byte)0x49, (byte)0x48, (byte)0x44, (byte)0x52,
		...
		// Image: tabbedPanel-corner-ls.png
		byte[]	CORNER_LS	=
		{
			(byte)0x89, (byte)0x50, (byte)0x4E, (byte)0x47, (byte)0x0D, (byte)0x0A, (byte)0x1A, (byte)0x0A,
			(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x0D, (byte)0x49, (byte)0x48, (byte)0x44, (byte)0x52,
		...
		// Image: tabbedPanel-corner-rs.png
		byte[]	CORNER_RS	=
		{
			(byte)0x89, (byte)0x50, (byte)0x4E, (byte)0x47, (byte)0x0D, (byte)0x0A, (byte)0x1A, (byte)0x0A,
			(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x0D, (byte)0x49, (byte)0x48, (byte)0x44, (byte)0x52,
...
////////////////////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////////////////
					...
					Action closeAction)
		{
			...
			height = TOP_MARGIN + Math.max(fontMetrics.getAscent() + fontMetrics.getDescent(),
										   CLOSE_BUTTON_HEIGHT) + BOTTOM_MARGIN;
	...
	////////////////////////////////////////////////////////////////////
		...
		protected void paintComponent(Graphics gr)
		{
			...
			if (fullWidth)
			{
				...
				gr.drawImage(selected ? CORNER_RS_ICON.getImage() : CORNER_R_ICON.getImage(),
							 width - CORNER_WIDTH, 0, null);
			}
			else
			{
				gr.fillRect(0, 0, CORNER_WIDTH, CORNER_HEIGHT);
			...
			if (!fullWidth)
			{
				...
				str = (maxTextWidth < fontMetrics.stringWidth(Constants.ELLIPSIS_STR))
									? null
			...
			if (str != null)
			{
				...
				gr.drawString(str, TEXT_LEADING_MARGIN,
	...
	////////////////////////////////////////////////////////////////////
		...
		private void showContextMenu(MouseEvent event)
		{
			if (event.isPopupTrigger())
			{
				...
				contextMenu.add(new FMenuItem(new TabAction(Command.CLOSE, Constants.CLOSE_STR)));
		...
		private void close(int modifiers)
		{
			if ((closeAction != null) && !closing)
			{
				...
				closeAction.actionPerformed(new ActionEvent(this, ActionEvent.ACTION_PERFORMED, command,
															modifiers));
				closing = false;
			}
		}
	...
	////////////////////////////////////////////////////////////////////
		...
		private MouseSelectionListWindow(Window owner)
		{
			...
			Collections.sort(orderedElements, elementComparator);
			list = new SelectionIndicatorList<>(orderedElements, (selectedIndex < 0)
																		? null
																		: elements.get(selectedIndex));
			...
			// Set location of window
			Point location = new Point(listButton.getWidth() - list.getPreferredSize().width,
	...
	////////////////////////////////////////////////////////////////////
		...
		public int compare(Element element1,
						   Element element2)
		{
			return (ignoreCase ? element1.toString().compareToIgnoreCase(element2.toString())
							   : element1.toString().compareTo(element2.toString()));
		}
...
////////////////////////////////////////////////////////////////////////
	...
	public TabbedPanel(Set<Border> borderComponents)
	{
	...
	public TabbedPanel(boolean directTabTraversal)
	{
	...
	public TabbedPanel(Set<Border> borders,
					   boolean     directTabTraversal)
	{
		...
		headerHeight = Math.max(HEADER_BUTTON_TOP_MARGIN + HEADER_BUTTON_HEIGHT +
																				HEADER_BUTTON_BOTTOM_MARGIN,
		...
		KeyboardFocusManager.getCurrentKeyboardFocusManager().
												addPropertyChangeListener(FOCUS_OWNER_PROPERTY_KEY, this);
...
////////////////////////////////////////////////////////////////////////
		...
		else
		{
			...
			for (int i = 0; i < getComponentCount(); i++)
			{
				...
				if (component instanceof Tab)
				{
					...
					int tabWidth = Math.min(component.getPreferredSize().width,
											maxTotalTabWidth - x);
					component.setBounds(x, getBorderSize(Border.TOP) + TOP_MARGIN + headerHeight -
																								tabHeight,
										tabWidth, tabHeight);
					x += tabWidth;
				}
			...
			for (int i = 0; i < elements.size(); i++)
			{
				...
				if (element.component.isVisible())
				{
					...
					int compWidth = Math.min(Math.max(minSize.width,
													  width - x - getBorderSize(Border.RIGHT)),
											 maxSize.width);
					int compHeight = Math.min(Math.max(minSize.height,
													   getHeight() - y -
																		getBorderSize(Border.BOTTOM)),
											  maxSize.height);
	...
	protected void paintComponent(Graphics gr)
	{
		...
		if (!elements.isEmpty())
		{
			...
			gr.drawLine(getBorderSize(Border.LEFT), y, getWidth() - getBorderSize(Border.RIGHT) - 1,
						y);
		}
	}
...
////////////////////////////////////////////////////////////////////////
	...
	public Dimension getFrameSize()
	{
		return new Dimension(getBorderSize(Border.LEFT) + getBorderSize(Border.RIGHT),
							 getBorderSize(Border.TOP) + TOP_MARGIN + headerHeight +
																		getBorderSize(Border.BOTTOM));
	}
	...
	{
		return Math.max(0, getWidth() - (getBorderSize(Border.LEFT) + MIN_HEADER_WIDTH +
																		getBorderSize(Border.RIGHT)));
	}
	...
	private void removeTabTraversalKeys(Component component)
	{
		if (directTabTraversal && (component instanceof JComponent))
			InputMapUtils.removeFromInputMap((JComponent)component, true,
