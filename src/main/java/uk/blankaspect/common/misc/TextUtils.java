...
////////////////////////////////////////////////////////////////////////
	...
	private static final	String	ESCAPE_STR	= Character.toString(ESCAPE_CHAR);
...
////////////////////////////////////////////////////////////////////////
										 ...
										 char   separator)
	{
		...
		return str.replace(ESCAPE_STR, ESCAPE_STR + ESCAPE_STR).replace(sep, ESCAPE_STR + sep);
	}
												 ...
												 RemovalMode removalMode)
	{
		...
		if (pathname != null)
		{
			...
			{
				// Create a list of pathname components.
				// If the pathname starts or ends with a separator, the separator is treated as belonging to
				// the first or last component respectively.
				...
				// Create a reduced pathname from the pathname components
				...
				switch (removalMode)
				{
					case START:
					{
						// Remove components from the pathname while it exceeds the maximum width
						index = 0;
						...
						// Concatenate an ellipsis and the remaining components
					...
					case MIDDLE:
					{
						// Find the widest and next widest pathname components, excluding the first and last
						// components
						int i0 = -1;
						int maxWidth0 = 0;
						int i1 = -1;
						...
						// Start with the widest component if it's at least twice as wide as the next
						// widest; otherwise, start with the middle component
						...
						// Concatenate the remaining components and an ellipsis
						if ((i0 > 0) || (i1 < stringWidths.size()))
						{
							for (int i = 0; i < i0; i++)
							{
				...
				// Remove characters from the most significant component
				if (outPathname == null)
				{
					String str = pathname;
					switch (removalMode)
