...
////////////////////////////////////////////////////////////////////////
	...
	public void parse(String... strs)
	{
		...
		for (String str : strs)
			Collections.addAll(inLines, str.split("\\n", -1));
			...
			for (Line.Element lineElement : line.elements)
			{
				switch (lineElement.getKind())
				{
								...
								case PADDING:
								{
									...
									if (paddingDef == null)
										throw new UndefinedTagException(Tag.HORIZONTAL_PADDING,
																		fieldElement.index);
									fieldElement.width = Math.round(paddingDef.width * (float)em);
									break;
								}
							}
					...
					case PADDING:
					{
						...
						lineElement.height = Math.round(paddingDef.height * (float)lineHeight);
						break;
					}
				}
			}
		...
		for (Line line : lines)
		{
			...
			else
			{
				for (Line.Element lineElement : line.elements)
				{
					...
					{
						Field field = (Field)lineElement;
						field.width = fieldWidths.containsKey(field.index)
																			? fieldWidths.get(field.index)
																			: field.elementWidth;
